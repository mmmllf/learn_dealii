...
void first_grid ()
{
  ...
  // cells in total:
  GridGenerator::hyper_cube (triangulation,0.0,64.0);
  triangulation.refine_global (2);

  const Point<2> origin (0,0);
  const double inner_radius = 32.0;
  ...
  for (unsigned int step=0; step<4; ++step)
      ...
      for (auto cell: triangulation.active_cell_iterators())
	{
	  for (unsigned int v=0;
		...
		= origin.distance(cell->vertex(v));
		
	      if (std::fabs(distance_from_origin - inner_radius) < 1e-10)
		{ 
